...
export default function ReportStatusPage() {
  ...
  if (!job) {
    return (
      <div className="private-main">
        <div style={{ color: 'var(--private-text-muted)', fontSize: '0.9rem' }}>Cargando...</div>
      </div>
    )
  }
    ...
    <div className="private-main">
      <div className="private-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <Link href="/social-listening" style={{ color: 'var(--private-text-muted)', fontSize: '0.85rem', textDecoration: 'none' }}>+ Nuevo reporte</Link>
        </div>
      </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '80px' }}>
                  <div 
                    ...
                    style={{
                      ...
                      backgroundColor: isError ? '#ff5050' : isDone ? 'var(--private-accent)' : isCurrent ? 'rgba(238,241,81,0.3)' : 'var(--private-border)',
                      border: isCurrent ? '2px solid var(--private-accent)' : isError ? '2px solid #ff5050' : '2px solid transparent',
                      color: isDone ? 'var(--private-bg)' : 'var(--private-text)',
                      fontSize: '0.9rem',
                      fontWeight: 700,
                  ...
                  <span style={{ fontSize: '0.7rem', marginTop: '0.5rem', color: isCurrent ? 'var(--private-accent)' : isDone ? 'var(--private-text)' : 'var(--private-text-muted)', textAlign: 'center', lineHeight: 1.3 }}>
                    {step.label}
                  </span>
                  {isCurrent && step.estimate && (
                    <span style={{ fontSize: '0.65rem', color: 'var(--private-text-muted)', marginTop: '0.2rem' }}>{step.estimate}</span>
                  )}
                </div>
                {i < steps.length - 1 && (
                  <div style={{ flex: 1, height: '2px', backgroundColor: isDone ? 'var(--private-accent)' : 'var(--private-border)', margin: '0 0.5rem', marginBottom: '2rem', transition: 'background-color 0.5s ease' }} />
                )}
              </div>
            )
          })}
      ...
      {/* Status messages */}
      <div style={{ padding: '1.5rem', backgroundColor: 'var(--private-card-bg)', border: '1px solid var(--private-border)', borderRadius: '4px' }}>
        {job.status === 'queued' && (
          <p style={{ color: 'var(--private-text)', fontSize: '0.9rem' }}>Esperando turno en el servidor...</p>
        )}
        
        {job.status === 'scraping_posts' && (
          <p style={{ color: 'var(--private-text)', fontSize: '0.9rem' }}>
        ...
        {job.status === 'error' && (
          <div>
            ...
            <p style={{ color: 'var(--private-text-muted)', fontSize: '0.9rem', marginBottom: '1rem' }}>{job.error ?? 'Error desconocido.'}</p>
            <button
              onClick={handleRetry}
              disabled={retrying}
              style={{
                padding: '0.6rem 1.5rem',
                backgroundColor: retrying ? 'var(--private-border)' : 'rgba(238,241,81,0.15)',
                color: retrying ? 'var(--private-text-muted)' : 'var(--private-accent)',
                border: '1px solid',
                borderColor: retrying ? 'var(--private-border)' : 'rgba(238,241,81,0.3)',
                borderRadius: '4px',
      ...
      {job.status === 'complete' && (
        <div style={{ 
          ...
          <p style={{ color: 'var(--private-text-muted)', marginBottom: '0.75rem', fontSize: '0.95rem' }}>
            El reporte de 8 páginas para {job.clientName} ha sido generado con éxito.
          </p>
          {job.generationCostUSD !== undefined && (
            <p style={{ color: 'var(--private-text-muted)', opacity: 0.5, fontSize: '0.75rem', marginBottom: '2rem', letterSpacing: '0.03em' }}>
      ...
      {(() => {
        ...
        return (
          <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid var(--private-border)' }}>
            <p style={{ fontSize: '0.65rem', color: 'var(--private-text-muted)', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Monitor Técnico</p>
              ...
              <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{
                    ...
                    backgroundColor: sincePolled !== null && sincePolled < 8 ? '#4ade80' : 'var(--private-border)',
                    boxShadow: sincePolled !== null && sincePolled < 8 ? '0 0 6px #4ade80' : 'none',
                    transition: 'all 0.5s ease',
                  }} />
                  <span style={{ fontSize: '0.75rem', color: 'var(--private-text-muted)' }}>
                    UI verificando
                    {sincePolled !== null && (
                      <span style={{ color: 'var(--private-text-muted)', opacity: 0.5, marginLeft: '0.3rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{
                    display: 'inline-block', width: '6px', height: '6px', borderRadius: '50%',
                    backgroundColor: looksStuck ? 'rgba(251,191,36,0.7)' : sinceUpdated < 30 ? '#4ade80' : 'var(--private-border)',
                  }} />
                  <span style={{ fontSize: '0.75rem', color: looksStuck ? 'rgba(251,191,36,0.8)' : 'var(--private-text-muted)' }}>
                    Servidor sin cambio
            ...
            {looksStuck && (
              <p style={{ fontSize: '0.75rem', color: 'rgba(251,191,36,0.8)', marginBottom: '0.75rem', lineHeight: 1.5 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
              <div style={{ fontSize: '0.7rem', color: 'var(--private-text-muted)', opacity: 0.6 }}>
                <span style={{ opacity: 0.5 }}>ID:</span> {reportId}
              </div>
              {job.apifyRunIds && Object.entries(job.apifyRunIds).map(([network, id]) => (
                <div key={id} style={{ fontSize: '0.7rem', color: 'var(--private-text-muted)', opacity: 0.6 }}>
                  <span style={{ opacity: 0.5 }}>Apify {network}:</span> {id}
                </div>
              ))}
              <div style={{ fontSize: '0.7rem', color: 'var(--private-text-muted)', opacity: 0.6 }}>
                <span style={{ opacity: 0.5 }}>Último webhook:</span> {new Date(job.updatedAt).toLocaleTimeString('es-CO')}
              </div>
            </div>
          </div>
        )
